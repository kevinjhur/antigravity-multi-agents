...
## 📉 토큰 절약 요청사항 (Token Saving)

- **에이전트 공통**: 모든 에이전트는 답변 시 '최소한의 출력으로 최대한의 정보'를 전달하는 것을 원칙으로 합니다.
- **코드 수정 시**: 전체 코드가 아닌 **Diff(차이점) 위주**로 응답하여 출력 토큰을 절약하세요.
- **작업 전 확인**: 모호한 요청이 있을 때는 임의로 전체 파일을 다시 쓰는 대신, 먼저 짧게 질문하여 범위를 좁히세요.